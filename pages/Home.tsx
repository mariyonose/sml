...
export const Home: React.FC<HomeProps> = ({ lang }) => {
  ...
  const slides = useMemo(() => {
    ...
    return [
      { id: 'brand-hero', isBrand: true, image: '', name: 'SSF Medical Link', title: 'Strategic Hub in Japan' },
        ...
        >
          ...
          <div className="md:hidden absolute inset-0 z-10 flex flex-col">
            ...
            >
              ...
              <div className="mb-2">
                <h1
                  className={`font-serif leading-[1.2] tracking-[-0.02em] ${slides[currentSlide].isBrand ? 'text-white' : 'text-slate-900'}`}
              ...
              {/* サブタイトル */}
              <motion.div key="sp-sub" className="mb-4" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.5 }}>
                <p className={`text-[11px] font-light leading-relaxed ${slides[currentSlide].isBrand ? 'text-blue-100/80' : 'text-slate-500'}`}>
                  {t('hero_subtitle')}
                </p>
              </motion.div>
              ...
              <div className="flex flex-row gap-3 mb-3">
                <Link to="/for-clinics" className="group relative flex-1 text-center px-3 py-3 bg-brand-blue overflow-hidden shadow-lg shadow-brand-blue/30 transition-transform active:scale-95">
                  <span className="relative z-10 text-white font-bold text-[11px] uppercase tracking-[0.15em] whitespace-nowrap">{t('nav_for_clinics')}</span>
                ...
                <Link to="/for-agents" className="flex-1 text-center px-3 py-3 bg-brand-teal font-bold text-[11px] uppercase tracking-[0.15em] whitespace-nowrap text-white transition-all duration-500 active:scale-95">
                  {t('nav_for_agents')}
                </Link>
              </div>
          ...
          <div className="hidden md:block relative z-10 w-full h-full">
            <div className="max-w-screen-2xl mx-auto px-10 xl:px-16 2xl:px-24 h-full flex items-center pb-16">
              ...
              <div className="flex flex-col gap-8 w-full max-w-[55%] xl:max-w-[50%] 2xl:max-w-[46%]">
                ...
                <motion.div key="pc-label" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} className="overflow-hidden">
                  <span className={`inline-block font-black tracking-[0.5em] uppercase text-sm ${slides[currentSlide].isBrand ? 'text-cyan-400' : 'text-brand-blue'}`}>
                    {slides[currentSlide].isBrand ? 'Authorized Medical Portal' : 'Certified Specialist Network'}
                  </span>
                </motion.div>
                ...
                <div className="flex flex-row gap-6 animate-in fade-in slide-in-from-bottom duration-1000 delay-700">
                  ...
                  <Link to="/for-agents" className={`px-10 py-5 border font-bold text-sm uppercase tracking-[0.4em] whitespace-nowrap transition-all duration-500 backdrop-blur-sm shadow-sm hover:-translate-y-1 ${slides[currentSlide].isBrand ? 'bg-brand-teal border-transparent text-white hover:bg-brand-teal' : 'bg-brand-teal border-transparent text-white hover:bg-slate-900'}`}>
                    {t('nav_for_agents')}
                  </Link>
                </div>
              </div>
            ...
            {/* 医師情報（右下に絶対配置） */}
            <motion.div
              key={`pc-profile-${currentSlide}`}
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
              className="absolute bottom-12 right-16 xl:right-20 2xl:right-28 z-20"
            >
              <div className={`border-r-2 pr-8 py-2 text-right ${slides[currentSlide].isBrand ? 'border-cyan-400' : 'border-brand-blue'}`}>
                <p className={`text-sm font-black uppercase tracking-widest mb-1 ${slides[currentSlide].isBrand ? 'text-cyan-400' : 'text-brand-blue'}`}>
